...
## Local configurations
    ...
    EVOMASTER_DIR = str(pathlib.PurePath("tools").as_posix()) #os.environ.get("EVOMASTER_DIR", "")
    EMB_DIR = str(pathlib.PurePath("suts").as_posix()) #os.environ.get('EMB_DIR',"")
...
def getConfigs():
   ...
   EXTRA_TARGETS = ParameterSetting("enableRPCExtraResponseTargets", [True])

   TEST_GEN = ParameterSetting("enablePureRPCTestGeneration", [True])
   ASSE_GEN = ParameterSetting("enableRPCAssertionWithInstance", [True])

   ### Example (step2) on how to define Config objects with specified ParameterSetting objects
   BASELINE = Config([ALGO_RANDOM, EXTRA_TARGETS, TEST_GEN, ASSE_GEN], "RPC_BASELINE")
   RPC_MIO = Config([ALGO_MIO, EXTRA_TARGETS, TEST_GEN, ASSE_GEN], "RPC_MIO")

   ### Example (step3) on employing Config objects for the experiments
   CONFIGS.append(BASELINE)
   CONFIGS.append(RPC_MIO)
